...
public:
  ...
  BOOL   TopologicalSort_Stack(); // 拓扑排序 // 对有向图的处理
  BOOL   TopologicalSort_Queue(); // 由队列生成的   因为拓扑排序不唯一
};
...
#define  STACKSIZESTEP 5  // 每一步的加和
#define  STACKDATA int		// 每次使用这个头文件的时候 我们就更改这里的数据类型就可以了
class STACK
{
...
public:
  ...
  BOOL DestroyStack();
  int GetStackLen();
  BOOL PushStack(STACKDATA Data);
  BOOL PopStack(STACKDATA &Data);
  BOOL GetTop(STACKDATA &Data);
  BOOL StackIsEmpty();
...
BOOL   ALG::TopologicalSort_Stack()
{
  ...
  for (i=0;i<m_iVexNum;++i)
  {
    InDegree[i] = GetVexInDegree(GetVex(i));
  }
  ...
  while (!S.StackIsEmpty())
  {
    ...
    for (pNodeTemp = m_Vertices[i].pFirstArc;pNodeTemp;pNodeTemp = pNodeTemp->pNextArc)
    {
       k = pNodeTemp->iAdjVex;
...
BOOL   ALG::TopologicalSort_Queue()
{
  int InDegree[MAX_VERTEX_NUM] = {0};
...
{
  if (NULL==(m_pData = (STACKDATA*)malloc(sizeof(STACKDATA)*STACKSIZESTEP)))
  {
    return FALSE;
  }
  
  memset(m_pData,0,sizeof(STACKDATA)*STACKSIZESTEP);
  m_nBase = 0;
  m_nTop = 0;
  m_nStackMAX = STACKSIZESTEP;
  return TRUE;
...
BOOL STACK::DestroyStack()
{
  ...
  m_nTop = m_nBase = m_nStackMAX = 0;
  return TRUE;
}
int STACK::GetStackLen()
{
  return m_nTop;
}
BOOL STACK::PushStack(STACKDATA Data)
{
  if (m_nTop==m_nStackMAX-1)
  {
    m_pData = (STACKDATA*)realloc(m_pData,sizeof(STACKDATA)*(m_nStackMAX+STACKSIZESTEP));
    ...
    memset(&m_pData[m_nStackMAX],0,sizeof(STACKDATA)*STACKSIZESTEP);
...
BOOL STACK::PopStack(STACKDATA &Data)
{
  if (StackIsEmpty())
  {
    Data = *(STACKDATA*)0;
...
BOOL STACK::GetTop(STACKDATA &Data)
{
  if (StackIsEmpty())
  {
    Data = *(STACKDATA*)0;
...
// 打印：
// 该图是12个顶点16条弧的有向图
// 输出12个顶点: 1 2 3 4 5 6 7 8 9 A B C
// 输出16条弧
// 1→3 1→C 1→2 1→4 2→3 3→5 3→8 3→7
// 
// 出度:1 入度:1 度:2
// 由栈生成的拓扑排序.
// 9 A B 6 1 4 2 3 8 5 7 C 是一个拓扑序列.
// 由队列生成的拓扑排序.
// 1 9 2 4 B A 3 6 C 5 8 7 是一个拓扑序列.