...
void SpaceList::InitSpaceList(pSp pSpaceTemp)
{
...
int SpaceList::MyMalloc(pSp pSpaceTemp)
{
...
void SpaceList::MyFree(pSp pSpaceTemp,int k)
{
	pSpaceTemp[k].Cur = pSpaceTemp[0].Cur;//判断只有一个结点的时候 是不是
	pSpaceTemp[0].Cur = k;
}

void SpaceList::Difference(pSp pSpaceTemp,int& S)
{
	InitSpaceList(pSpaceTemp);
...
void SpaceList::TravelList(pSp pSpaceTemp,int S)//S 是Difference里面的头 
{
